...
func (provider *AlertProvider) Send(alert *alert.Alert) error {
	...
	subject, body := provider.buildMessageSubjectAndBody(alert)
	m := gomail.NewMessage()
...
func (provider *AlertProvider) buildMessageSubjectAndBody(alert *alert.Alert) (string, string) {
	var subject, message string
	if alert.Subject != nil && len(*alert.Subject) > 0 {
		subject = *alert.Subject
	}
	if alert.Description != nil && len(*alert.Description) > 0 {
...
func (provider *AlertProvider) ValidateOverrides(alert *alert.Alert) error {
	_, err := provider.GetConfig(alert)
	return err
}
